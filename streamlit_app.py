...
<div style='background:#EFF6FF;border:1.5px solid #BFDBFE;border-radius:10px;
  ...
  <div style='font-size:13px;font-weight:800;color:#1F3A5F;margin-bottom:6px'>
    📋 המצב הקיים בטאב זה — {_tab_snap.get("count",0)} מוצרים · {_ils_total} · {_label_multi}
  </div>
  <div style='display:flex;gap:16px;flex-wrap:wrap;font-size:12px'>
    ...
    <span>🔒 לא-סחיר: <b>{_fmt_nan(_tab_snap.get("illiquid"))}</b></span>
        ...
        # ── Bonus: Load current state to sliders ──
        if st.button(
            "📥 טען מצב קיים לסליידרים",
            key="btn_load_tab_to_sliders",
            help="ממלא את יעדי התמהיל בערכי המצב הקיים של הטאב",
            use_container_width=True,
        ):
            _tgts = st.session_state.get("targets", {})
            _changed = False
            for _k, _snap_k in [("stocks","equity"),("foreign","foreign"),("fx","fx"),("illiquid","illiquid")]:
                _v = _tab_snap.get(_snap_k, float("nan"))
                if not math.isnan(_v):
                    _tgts[_k] = round(float(_v), 1)
                    _changed = True
            if _changed:
                st.session_state["targets"] = _tgts
                st.rerun()
...
<div style='background:#fff;border:1px solid #E5EAF2;border-radius:10px;
  ...
  <div style='font-size:12px;font-weight:800;color:#1F3A5F;margin-bottom:8px'>
    🔄 השפעה משוערת על כלל הפורטפוליו
  </div>
