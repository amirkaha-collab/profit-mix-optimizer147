...
def _render_debug(df_yearly, df_monthly, debug_info, errors):
    with st.expander("🛠️ מידע אבחון (debug)", expanded=False):
        ...
        # ── Manager normalization table ────────────────────────────────────
        combined = pd.concat([df_yearly, df_monthly], ignore_index=True) \
            if not df_yearly.empty or not df_monthly.empty else pd.DataFrame()
        if not combined.empty and "source_sheet" in combined.columns:
            st.markdown("**🏷️ זיהוי גופים מנהלים**")
            mgr_rows = []
            for sheet in combined["source_sheet"].unique():
                sub = combined[combined["source_sheet"] == sheet]
                canonical = sub["manager"].iloc[0] if "manager" in sub.columns else "—"
                match_type = sub["manager_match"].iloc[0] if "manager_match" in sub.columns else "—"
                badge = {"exact": "✅ מדויק", "fuzzy": "🔶 מקורב", "unresolved": "❓ לא זוהה"}.get(
                    match_type, match_type
                )
                mgr_rows.append({
                    "גליון מקור": sheet,
                    "שם גוף (גולמי)": sheet,
                    "שם גוף (נרמל)": canonical,
                    "אופן זיהוי": badge,
                })
...
def render_institutional_analysis(
    ...
    with st.expander("📐 ניתוח אסטרטגיות מוסדיים", expanded=False):
        ...
        if _ctx_product_type is not None:
            with st.expander("🔍 הקשר מהאפליקציה הראשית", expanded=False):
                df_info = (
                    f"כן ({len(_ctx_df_long):,} שורות)"
                    if _ctx_df_long is not None and not _ctx_df_long.empty
                    else ("כן (ריק)" if _ctx_df_long is not None else "לא")
                )
                st.caption(
                    f"**סוג מוצר פעיל:** {_ctx_product_type}  \n"
                    f"**df_long התקבל:** {df_info}"
                )
        ...
        # Debug expander — shown below filter UI, collapsed by default
        with st.expander("🔍 אבחון — גופים מנהלים", expanded=False):
            st.caption(
                f"**גופים ב-ISA sheet:** {len(isa_managers)}  \n"
                f"**גופים מהטאב הפעיל:** {len(extra_canonical)}  \n"
                f"**סך גופים בתפריט:** {len(full_manager_universe)}  \n"
                f"**שמות לא מזוהים (unresolved):** "
                + (", ".join(unresolved) if unresolved else "אין")
            )
            if tab_managers_norm:
                import pandas as _pd_dbg
                st.dataframe(
                    _pd_dbg.DataFrame([
                        {"שם גולמי": r, "שם נרמל": c, "סוג התאמה": t}
                        for r, (c, t) in tab_managers_norm.items()
                    ]),
                    use_container_width=True, hide_index=True,
                )
